...
class TestCleanFunctions(unittest.TestCase):
    def test_age_filter(self) -> None:
        ...
        df_expected = spark.createDataFrame(
            [("Cussac", 27, 75020), ("davis", 18, 75020)], ["name", "age", "zip"]
        )
        expected_list = [
    ...
    def test_join(self) -> None:
        ...
        # Then
        df_expected = spark.createDataFrame(
            [("Cussac", 27, 75020, "Paris")], ["name", "age", "zip", "city"]
        )
        expected_list = [
            (row["name"], row["age"], row["zip"], row["city"])
    ...
    def test_departement_format(self) -> None:
        ...
        # Given
        df_clients = spark.createDataFrame(
            [("Culdusac", 27, 75020), ("david", 40, 972), ("eti", 40, 1)],
            ["name", "age", "zip"],
        )

        df_city = spark.createDataFrame(
            [(75020, "Paris"), (972, "Martinique"), (1, "first")], ["zip", "city"]
        )

        # When
        df_result = departement_format(my_join(df_clients, df_city, "zip"))
        actual_list = [
            (row["name"], row["age"], row["zip"], row["city"], row["departement"])
            for row in df_result.collect()
        ]
